...
module Kdeploy
  module Web
    ...
    module ExecutionAdapter
      ...
      class << self
        def execute(task_file_path:, task_name:, limit:, parallel:, retries:, retry_delay:, format:, on_step: nil)
          defaults = default_variables_for(task_file_path)
          runtime = build_runtime(task_file_path, defaults)
          selected_task =
          ...
          step_timeout = env_float('JOB_CONSOLE_STEP_TIMEOUT') || Kdeploy::Configuration.default_step_timeout
          retry_on_nonzero = env_bool('JOB_CONSOLE_RETRY_ON_NONZERO', default: Kdeploy::Configuration.default_retry_on_nonzero)
          retry_policy = Kdeploy::Configuration.default_retry_policy
        ...
        def build_runtime(task_file_path, defaults)
          path = ensure_task_path_allowed!(task_file_path)
          runtime = Class.new(Runtime)
          apply_defaults(runtime, defaults)
        ...
        def serialize_result(result)
          {
            ...
            error: result[:error],
            steps: Array(result[:output]).map { |s| serialize_step(s) }
          }
        end
        ...
        def default_variables_for(task_file_path)
          job = Models::Job.first(task_file_path: task_file_path)
          defaults = job&.default_variables
          defaults.is_a?(Hash) ? defaults : {}
        end

        def apply_defaults(runtime, defaults)
          defaults.each do |key, value|
            name = key.to_s
            next unless name.match?(/\A[a-zA-Z_]\w*\z/)
            next if runtime.respond_to?(name)
